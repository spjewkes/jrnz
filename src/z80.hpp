	...
	Z80(unsigned int ram_size, std::string &rom_file) : mem(ram_size, rom_file)
		{
			...
			map_inst.emplace(0x35, Instruction{InstType::DEC, "dec (hl)", 1, 11, Operand::indHL1, Operand::ONE});
			map_inst.emplace(0x36, Instruction{InstType::LD,  "ld (hl),*", 2, 10, Operand::indHL1, Operand::N});
