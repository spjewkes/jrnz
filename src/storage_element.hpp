/**
 * @brief Defines the storage element class which is used to manage operations
 * generically.
 */
...
/**
 * @brief Defines a standard storage element type to allow operations between
 * registers/memory.
 */
class StorageElement {
 public:
  ...
  // Compiler warns about overriding the default copy constructor here. This
  // needs resolving as removing it will break the emulator
  StorageElement &operator=(const StorageElement &rhs);
  StorageElement &operator=(const uint8_t rhs);
  StorageElement operator+(const StorageElement &rhs);
  static StorageElement add_carry(const StorageElement &lhs,
                                  const StorageElement &rhs, bool carry);
  StorageElement operator-(const StorageElement &rhs);
  static StorageElement sub_carry(const StorageElement &lhs,
                                  const StorageElement &rhs, bool carry);
  StorageElement &operator^=(const StorageElement &rhs);
  StorageElement &operator&=(const StorageElement &rhs);
  StorageElement &operator|=(const StorageElement &rhs);
  ...
  bool is_even_parity() const {
    return (std::bitset<32>(to_u32()).count() % 2) == 0;
  }
 ...
 private:
  ...
  void update_carry(const StorageElement &op1, const StorageElement &op2,
                    bool is_half = false);
  void update_borrow(const StorageElement &op1, const StorageElement &op2,
                     bool is_half = false);
  ...
  std::vector<uint8_t>
      read_only;  // This should only be used if the readonly flag is set
  bool readonly;  //! TODO - this should be a specialized version of
                  //! StorageElement
};
